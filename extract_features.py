...
for fruit_dir in fruits:
    ...
    dataset_features = np.zeros(shape=(len(all_imgs), 360))
    outputs = np.zeros(shape=(len(all_imgs)))
    ...
    np.random.shuffle(dataset_features)
    np.save("data/"+fruit_dir+"_dataset.npy", dataset_features)
    np.save("data/"+fruit_dir+"_outputs.npy", outputs)
